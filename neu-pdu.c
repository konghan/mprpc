...
neupdu_t *neupdu_chunk_new(neusession_t *ses, int size, int slice, int header){
    ...
    if(header != 0){
        ...
        pdu->iovs[0].iov_data = xmalloc(header);
        if(pdu->iovs[0].iov_data == NULL){
            xfree(pdu);
            return NULL;
        }
        len = size - header;
    ...
    for(; i < num; i++){
        pdu->iovs[i].iov_len = min(len, slice);
        pdu->iovs[i].iov_data = xmalloc(pdu->iovs[i].iov_len);
        if(pdu->iovs[i].iov_data == NULL)
            goto err_out;

        len -= slice;
    }
    
    pdu->header = (neupdu_header_t*)pdu->iovs[0].iov_data;
    pdu->num = num;
    pdu->ses = ses;
    pdu->start = pdu->end = 0;
