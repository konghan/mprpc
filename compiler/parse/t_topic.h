...
/**
 * A topic is a container for a set of member fields that has a name. Topics
 * are also used to implement exception types.
 *
 */
class t_topic : public t_type {
 public:
  typedef std::vector<t_field*> members_type;

  t_topic(t_program* program) :
    t_type(program),
    is_xception_(false),
    is_union_(false),
  ...
  t_topic(t_program* program, const std::string& name) :
    t_type(program, name),
    is_xception_(false),
    is_union_(false),
    members_validated(false),
